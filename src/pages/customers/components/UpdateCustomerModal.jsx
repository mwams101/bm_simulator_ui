...
const UpdateCustomerModal = ({ isOpen, onClose, onUpdated, customer }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const r = await fetch(`http://localhost:8000/new-bank-customers/${customer.id}`, {
                method: 'PUT',
            ...
            if (!r.ok) { const d = await r.json(); throw new Error(d.detail || 'Failed to update customer'); }
            onUpdated(); onClose();
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center py-6 px-4">
            ...
            <div className="relative z-10 w-full max-w-[680px] bg-white dark:bg-[#161e2e] rounded-2xl shadow-2xl border border-[#e7ebf3] dark:border-gray-800 overflow-hidden max-h-[90vh] flex flex-col">
                <div className="px-8 pt-8 pb-5 border-b border-[#f0f2f5] dark:border-gray-800 flex items-center justify-between shrink-0">
                    <div>
                        <h1 className="text-xl font-black text-[#0d121b] dark:text-white tracking-tight">Edit Customer</h1>
                        <p className="text-[#4c669a] dark:text-gray-400 text-sm mt-0.5">Editing: {customer.first_name} {customer.last_name}</p>
                    </div>
                ...
                <div className="overflow-y-auto flex-1">
                    {error && <div className="mx-8 mt-5 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg flex gap-2 items-start"><span className="material-symbols-outlined text-red-500 text-sm">error</span><p className="text-sm text-red-600 dark:text-red-400">{error}</p></div>}
                    <form onSubmit={handleSubmit} className="p-8 space-y-5">
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <Field label="Phone (masked)"><input className={inp} required value={form.phone_masked || ''} onChange={e => set('phone_masked', e.target.value)} disabled={loading} /></Field>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <Field label="Status">
                                <select className={inp} value={form.status || 'ACTIVE'} onChange={e => set('status', e.target.value)} disabled={loading}>
